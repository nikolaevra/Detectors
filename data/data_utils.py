...
def preprocess_true_boxes(true_boxes, true_labels, input_shape, anchors, num_classes):
    ...
    y_true_13 = np.zeros(shape=[grid_sizes[0][0], grid_sizes[0][1], 3, 5 + num_classes],
                         dtype=np.float32)
    y_true_26 = np.zeros(shape=[grid_sizes[1][0], grid_sizes[1][1], 3, 5 + num_classes],
                         dtype=np.float32)
    y_true_52 = np.zeros(shape=[grid_sizes[2][0], grid_sizes[2][1], 3, 5 + num_classes],
                         dtype=np.float32)
...
def convert_yolo_to_min_max_box(x_center, y_center, width, height, img_height, img_width):
...
def convert_yolo_to_min_max_boxes(bboxes, img_height, img_width):
    for i in range(bboxes.shape[0]):
        bboxes[i] = convert_yolo_to_min_max_box(
            bboxes[i][0],
            bboxes[i][1],
            bboxes[i][2],
            bboxes[i][3],
...
def load_detections_into_dict(file_path):
    with open(file_path, 'r') as f:
        lines = f.readlines()
        bboxes = np.zeros((len(lines), 4), dtype=np.float32)
        labels = np.zeros((len(lines),), dtype=np.int64)

        for i, line in enumerate(lines):
            line = line.rstrip('\n')
            if len(line) > 0:
                detection = line.split(' ')
                labels[i] = detection[0]
                bboxes[i] = detection[1:]

    return bboxes, labels


def read_image_box_from_text(text_path, base_dir):
    """
    :param text_path
    ...
    with open(text_path, 'r') as f:
        for line in f.readlines():
            ...
            data[os.path.join(base_dir, line)] = load_detections_into_dict(file_path)
...
class Parser(object):
    ...
    def preprocess(self, image, true_labels, true_boxes):
        ...
        y_true_13, y_true_26, y_true_52 = tf.py_func(preprocess_true_boxes,
                                                     inp=[true_boxes, true_labels, self.input_shape,
...
def bbox_iou(a_box, b_box):
    intersect_mins = np.maximum(a_box[:, 0:2], b_box[:, 0:2])
    intersect_maxs = np.minimum(a_box[:, 2:4], b_box[:, 2:4])
    intersect_wh = np.maximum(intersect_maxs - intersect_mins, 0.)
    intersect_area = intersect_wh[..., 0] * intersect_wh[..., 1]

    a_area = np.prod(a_box[:, 2:4] - a_box[:, 0:2], axis=1)
    b_area = np.prod(b_box[:, 2:4] - b_box[:, 0:2], axis=1)
...
def get_prediction_boxes(y_pred, num_classes):
    pred_boxes = y_pred[0]
    pred_confs = y_pred[1]
    pred_probs = y_pred[2]
...
def evaluate(y_pred, y_true, num_classes, score_thresh=0.5, iou_thresh=0.5, num_feature_maps=3):
    ...
    for i in range(num_images):
        ...
        for j in range(num_feature_maps):  # go over all feature maps
        ...
        for k in range(len(true_labels_list)):
            ...
            max_iou_idx = np.argmax(iou)  # Extract index of largest overlap
            avg_iou += iou[max_iou_idx]

            if (iou[max_iou_idx] >= iou_thresh and
                    true_labels_list[k] == pred_labels_list[max_iou_idx]
                    and max_iou_idx not in detected):
                pred_labels_dict[true_labels_list[k]] += 1
                detected.append(max_iou_idx)

        pred_labels_list = [pred_labels_list[max_iou_idx] for max_iou_idx in detected]
        per_image_iou.append(avg_iou / len(true_labels_list))
    ...
    avg_iou = sum(per_image_iou) / num_images

    return recall, precision, mAP, avg_iou
