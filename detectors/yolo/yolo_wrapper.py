...
import yolo_v3
import yolo_v3_tiny
from module_utils.timer import Timer
from yolo_utils import load_coco_names, draw_boxes, detections_boxes, non_max_suppression


class YoloDetectorWrapper:
    def __init__(self, tiny=False, cls_file='coco.names', img_size=(416, 416), data_format='NHWC',
                 ckpt_file='saved_model/model.ckpt', conf_threshold=0.5, iou_threshold=0.4):
        ...
        if self.tiny:
            self.ckpt_file = path.join(self.BASE_DIR, ckpt_file.split('.')[0] + '-tiny.ckpt')
        else:
            self.ckpt_file = path.join(self.BASE_DIR, ckpt_file)
        ...
        self.classes = load_coco_names(path.join(self.BASE_DIR, cls_file))

        self.inputs = tf.placeholder(tf.float32, [1, self.size[0], self.size[1], 3])

        with tf.variable_scope('detector'):
            self.detections = self.model(self.inputs, len(self.classes),
    ...
    def detect(self, image, output_img='out'):
        ...
        img.save((output_img if not self.tiny else output_img + '-tiny') + '.jpg')
